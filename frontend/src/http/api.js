...
const PROTOTYPE_LOCAL_PREFIXES = ['dp_inspiration_orbs_v1:', 'dp_fellow_profile_v2:'];
const PROTOTYPE_SESSION_KEYS = ['daymark_focus_timer_v2', 'daymark_today_tasks_v1'];
const PROTOTYPE_SESSION_PREFIXES = ['daymark_focus_timer_v2:', 'daymark_today_tasks_v1:', 'dp_song_cache_v2:'];
...
export function clearPrototypeLocalState() {
  try {
    ...
    Object.keys(window.localStorage)
      .filter((key) => PROTOTYPE_LOCAL_PREFIXES.some((prefix) => key.startsWith(prefix)))
      .forEach((key) => window.localStorage.removeItem(key));
    PROTOTYPE_SESSION_KEYS.forEach((key) => window.sessionStorage.removeItem(key));
    Object.keys(window.sessionStorage)
      .filter((key) => PROTOTYPE_SESSION_PREFIXES.some((prefix) => key.startsWith(prefix)))
...
export async function loginDemoWorkspace() {
  return apiRequest(API_ENDPOINTS.SESSION_DEMO, {
    method: 'POST',
  });
}
