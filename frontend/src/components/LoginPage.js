...
function LoginPage() {
  ...
  const demoCopy = lang === 'zh'
    ? {
        label: 'Showcase workspace',
        title: '直接进 fellowship demo 工作区',
        body: '自动载入一份已经准备好的示例账号，里面有官方 milestones、review 数据和接下来要推进的 showcase 任务。',
        action: '进入 demo',
      }
    : {
        label: 'Showcase workspace',
        title: 'Enter the fellowship demo workspace',
        body: 'Load a ready-made account with official milestones, seeded review data, and the next showcase tasks to move.',
        ...
        <section className="animate-slide-up overflow-hidden">
            ...
            <div>
              ...
              <div className="mt-4">
                <BrandMark
                  ...
                  subtitle={
                    lang === 'zh'
                      ? '把 fellowship 时间线、个人执行和复盘放进同一块真实时间坐标里。'
                      : 'Map the fellowship timeline, personal execution, and review into one operating board.'
                  }
                />
              </div>
            </div>
          ...
          <div className="mt-5 max-w-[840px]">
            ...
            <div className="max-w-none pt-2 pr-2">
              ...
              <h1 className="max-w-[15ch] text-[clamp(1.14rem,1.7vw,1.58rem)] leading-[1.02] text-[color:var(--text)]">
                {lang === 'zh' ? (
                  '把 fellowship 的任务和节奏压成更清楚的一条轨迹。'
                ) : (
                  <>
                    Turn the fellowship timeline
                    <br />
                    into a usable operating trace.
                  </>
                )}
              </h1>
          ...
          <section className="board-card animate-slide-up mt-5 mx-auto max-w-[760px] p-4 md:p-5">
            ...
            <div className="mt-5 rounded-[22px] border border-[color:var(--line)] bg-[color:var(--surface-muted)] p-4">
              <div className="panel-label">{demoCopy.label}</div>
              <div className="mt-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="max-w-[32rem]">
                  <h3 className="text-lg text-[color:var(--text)]">{demoCopy.title}</h3>
                  <p className="mt-2 text-sm leading-6 text-[color:var(--muted)]">{demoCopy.body}</p>
                </div>
                <button
                  type="button"
                ...
                >
                  {demoLoading ? t.loginLoading : demoCopy.action}
                </button>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="mt-5 space-y-3.5">
              ...
              <button
                type="submit"
                disabled={submitting || demoLoading || !displayName.trim() || !password.trim()}
