...
const TASK_CACHE_KEY_PREFIX = 'daymark_today_tasks_v1';
...
function taskCacheKey(userId) {
  ...
  return `${TASK_CACHE_KEY_PREFIX}:${suffix}`;
}

function readTaskCache(userId) {
  try {
    const raw = window.sessionStorage.getItem(taskCacheKey(userId));
    if (!raw) return null;
...
function writeTaskCache(userId, tasks) {
  try {
    window.sessionStorage.setItem(taskCacheKey(userId), JSON.stringify(tasks || []));
  } catch {}
}
...
function DailyPlan({ planRefreshSignal = 0, taskRefreshSignal = 0, onPlanUpdated, inspirations = [], onDismissInspiration }) {
  ...
  }, [lang, loadTasks, mergeTaskLocally, removeTaskLocally, tasks, userId]);
