...
import FellowshipTodayPanel from './FellowshipTodayPanel';
...
const INSPIRATION_STORAGE_KEY_PREFIX = 'dp_inspiration_orbs_v1';

function inspirationStorageKey(userId) {
  const suffix = userId ? String(userId) : 'guest';
  return `${INSPIRATION_STORAGE_KEY_PREFIX}:${suffix}`;
}

function readInspirations(userId) {
  try {
    const stored = window.localStorage.getItem(inspirationStorageKey(userId));
...
function TodayPage() {
  ...
  const [inspirations, setInspirations] = useState(() => readInspirations(userId));
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(inspirationStorageKey(userId), JSON.stringify(inspirations));
    } catch {}
  }, [inspirations, userId]);

  useEffect(() => {
    setInspirations(readInspirations(userId));
  }, [userId]);
  ...
  return (
    <div className="space-y-4">
      <section className="board-card px-5 py-5">
        <div className="panel-label">{t.todayEyebrow}</div>
