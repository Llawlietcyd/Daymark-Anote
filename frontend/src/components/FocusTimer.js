...
const STORAGE_KEY_PREFIX = 'daymark_focus_timer_v2';

function focusTimerStorageKey(userId) {
...
function FocusTimer({ tasks = [], selectedTaskId, onSelectedTaskChange = () => {}, compact = false }) {
  ...
  const storageKey = useMemo(() => focusTimerStorageKey(userId), [userId]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      setUseCustom(false);
      setPhase('work');
      setRemainingSeconds(PRESETS[0].work * 60);
      setIsRunning(false);
