...
from core.fellowship_demo import seed_fellowship_demo_workspace
...
def login_demo_workspace():
    with get_db() as db:
        user = seed_fellowship_demo_workspace(db)
        session_token = secrets.token_urlsafe(32)
        db.add(UserSession(user_id=user.id, token=session_token))
        db.flush()
        current = {
        ...
        onboarding = read_setting(
            db,
            onboarding_key(user.id),
            {"completed": True, "daily_capacity": 6, "profile_summary": "Seeded fellowship showcase workspace."},
        )
        return _session_response(current, onboarding)
