...
from core.fellowship_program import (
    FELLOWSHIP_DATES,
...
def _suggested_prompts(lang: str, profile_completed: bool, pending: Dict[str, Any]) -> List[str]:
    ...
    return (
        [
            "中期 check-in 是什么时候",
            "Anote fellowship 的 3 个 deliverables 是什么",
            "分析一下我最近的专注和完成情况",
        ]
        if lang == "zh"
        else [
            "When is the midpoint check-in?",
            "What are the 3 fellowship deliverables?",
...
def _has_fellowship_context(text: str) -> bool:
...
_WEEKDAY_TO_RECURRING = {2: 0, 4: 1, 5: 2}


def _fellowship_events_for_date(iso_day: str) -> List[Dict[str, Any]]:
    start = FELLOWSHIP_DATES["start"]
    end = FELLOWSHIP_DATES["end"]
    if not (start <= iso_day <= end):
        return []
    ...
    if weekday in _WEEKDAY_TO_RECURRING:
        recurring = FELLOWSHIP_RECURRING_EVENTS[_WEEKDAY_TO_RECURRING[weekday]]
        events.append(
            {
                "label": recurring["label"],
                "when": recurring["cadence"],
                "notes": recurring["notes"],
            }
        )
    for fixed in FELLOWSHIP_FIXED_EVENTS:
        if fixed["date"] == iso_day:
            events.append(
                {
                    "label": fixed["label"],
                    "when": fixed["time"],
...
def _fellowship_program_reply(message: str, lang: str) -> Dict[str, Any] | None:
    normalized = (message or "").strip()
    lower = normalized.lower()
    if not normalized or _has_explicit_command_intent(normalized):
        return None
    has_fellowship_context = _has_fellowship_context(lower)

    asks_today_events = _contains_any(
        lower,
        [
            "today in the fellowship",
            "fellowship today",
            "any fellowship event today",
            "今天 fellowship",
            "今天的 fellowship",
            "fellowship 今天",
            "今天有什么 fellowship",
    ...
    if asks_today_events:
        today_iso = local_today_iso()
        events = _fellowship_events_for_date(today_iso)
        if not events:
            reply = (
                f"Fellowship 日历里 {today_iso} 没有固定安排，你今天可以专注在自己的 paper 或 product sprint 上。"
                if lang == "zh"
                else f"No fixed fellowship event on {today_iso}. Today is yours for paper or product work."
            )
        else:
            lines = [f"- {e['label']} ({e['when']}) — {e['notes']}" for e in events]
            if lang == "zh":
                reply = f"{today_iso} 的 fellowship 安排：\n" + "\n".join(lines)
            else:
                reply = f"Fellowship events on {today_iso}:\n" + "\n".join(lines)
        return {"reply": reply, "requires_clarification": False, "clarification_question": "", "actions": []}

    asks_this_week = _contains_any(
        lower,
        [
    ...
    if asks_this_week:
        ...
        for offset in range(7):
            ...
            iso_d = d.isoformat()
            events = _fellowship_events_for_date(iso_d)
            if events:
                labels = ", ".join(e["label"] for e in events)
                lines.append(f"- {iso_d} ({d.strftime('%a')}): {labels}")
        if not lines:
            reply = (
                "本周 fellowship 日历里没有命中固定事件，看起来是自主工作周。"
                if lang == "zh"
                else "No fellowship events fall in this week — it's a self-driven sprint week."
            )
        else:
            header = "本周 fellowship 节奏：" if lang == "zh" else "This week in the fellowship:"
            reply = header + "\n" + "\n".join(lines)
        return {"reply": reply, "requires_clarification": False, "clarification_question": "", "actions": []}

    asks_deliverables = (
        has_fellowship_context
        and _contains_any(
            lower,
            [
                "deliverables",
        ...
        lower,
        [
            "3 fellowship deliverables",
            "three fellowship deliverables",
            "3 个 fellowship deliverables",
            "3个 fellowship deliverables",
            "3 个交付物",
            "3个交付物",
        ],
    )
    asks_overview = has_fellowship_context and _contains_any(
        lower,
        [
            "schedule",
            "calendar",
    ...
    ) and _contains_any(lower, ["fellowship", "program", "anote", "项目"])

    if asks_deliverables:
        if lang == "zh":
            reply = (
                "Anote AI Research Fellowship 有 3 个核心交付物："
                f"1. Research paper，目标是在 {FELLOWSHIP_DELIVERABLES[0]['date']} 前完成并投稿；"
                f"2. Final presentation，在 {FELLOWSHIP_DELIVERABLES[1]['date']} 做最终展示；"
                f"3. Product contribution，在 {FELLOWSHIP_DELIVERABLES[2]['date']} 前把你负责的 Anote 产品改出真正可交付的结果。"
            )
        else:
            reply = (
                "The Anote AI Research Fellowship has 3 core deliverables: "
                f"1. a research paper due by {FELLOWSHIP_DELIVERABLES[0]['date']}; "
                f"2. a final presentation on {FELLOWSHIP_DELIVERABLES[1]['date']}; "
                f"3. a meaningful product contribution shipped by {FELLOWSHIP_DELIVERABLES[2]['date']}."
            )
        return {"reply": reply, "requires_clarification": False, "clarification_question": "", "actions": []}

    if asks_dates:
        reply = (
            f"Anote AI Research Fellowship runs from {FELLOWSHIP_DATES['start']} to {FELLOWSHIP_DATES['end']}."
            if lang != "zh"
            else f"Anote AI Research Fellowship 的项目周期是 {FELLOWSHIP_DATES['start']} 到 {FELLOWSHIP_DATES['end']}。"
        )
        return {"reply": reply, "requires_clarification": False, "clarification_question": "", "actions": []}

    recurring_event_queries = [
        (FELLOWSHIP_RECURRING_EVENTS[0], ["research standup", "ai research standup", "研究 standup", "研究站会"]),
        (FELLOWSHIP_RECURRING_EVENTS[1], ["product standup", "ai product standup", "产品 standup", "产品站会"]),
        (FELLOWSHIP_RECURRING_EVENTS[2], ["fellowship talk", "friday fellowship talk", "fellowship 分享"]),
    ]
    fixed_event_queries = [
        (FELLOWSHIP_FIXED_EVENTS[0], ["fellowship orientation", "orientation & welcome", "anote orientation", "开营", "迎新"]),
        (FELLOWSHIP_FIXED_EVENTS[1], ["midpoint", "midpoint check-in", "midpoint checkins", "midpoint one-on-one", "midpoint one on one", "midpoint 1:1", "中期 check-in", "中期一对一", "中期"]),
        (FELLOWSHIP_FIXED_EVENTS[2], ["fellowship final presentations", "final presentations", "fellowship showcase", "最终展示", "最后展示"]),
        (FELLOWSHIP_FIXED_EVENTS[3], ["anote summer party", "summer party", "夏日派对"]),
        (FELLOWSHIP_FIXED_EVENTS[4], ["fellowship last day", "final wrap-up", "last day", "最后一天"]),
    ]

    for event, strong_keywords in fixed_event_queries:
        if _contains_any(lower, strong_keywords) or (has_fellowship_context and _contains_any(lower, event["keywords"])):
            label = pick_event_field(event, "label", lang)
            time_str = pick_event_field(event, "time", lang)
            notes = pick_event_field(event, "notes", lang)
            if lang == "zh":
                reply = f"{label} — {event['date']}，{time_str}。{notes}"
            else:
                reply = f"{label} — {event['date']}, {time_str}. {notes}"
            return {"reply": reply, "requires_clarification": False, "clarification_question": "", "actions": []}

    for event, strong_keywords in recurring_event_queries:
        if _contains_any(lower, strong_keywords) or (has_fellowship_context and _contains_any(lower, event["keywords"])):
            label = pick_event_field(event, "label", lang)
            cadence = pick_event_field(event, "cadence", lang)
            notes = pick_event_field(event, "notes", lang)
            if lang == "zh":
                reply = f"{label} — 固定节奏：{cadence}。{notes}"
            else:
                reply = f"{label} — recurring: {cadence}. {notes}"
            return {"reply": reply, "requires_clarification": False, "clarification_question": "", "actions": []}

    if asks_overview or _contains_any(lower, ["anote fellowship", "anote ai research fellowship"]):
        if lang == "zh":
            reply = (
                f"这个 fellowship 从 {FELLOWSHIP_DATES['start']} 到 {FELLOWSHIP_DATES['end']}。"
                "固定节奏是：每周二 research standup、每周四 product standup、每周五 fellowship talk。"
                "几个关键节点是 2026-06-01 orientation、2026-07-01 中期 one-on-one、2026-08-05 final presentations、2026-08-10 final wrap-up。"
            )
        else:
            reply = (
                f"The fellowship runs from {FELLOWSHIP_DATES['start']} to {FELLOWSHIP_DATES['end']}. "
                "Its recurring rhythm is Tuesday research standup, Thursday product standup, and Friday fellowship talk. "
                "Key milestones are orientation on 2026-06-01, midpoint check-ins on 2026-07-01, final presentations on 2026-08-05, and final wrap-up on 2026-08-10."
            )
        return {"reply": reply, "requires_clarification": False, "clarification_question": "", "actions": []}
...
def _extract_task_date_question_query(message: str) -> str:
    ...
    cleaned = re.sub(r"^(我的|我那个|那个|这个|任务[:：]?\s*|task[:：]?\s*|my\s+)", "", cleaned, flags=re.IGNORECASE)
...
"""
    response_lang_note = (
        "Respond in Chinese (中文). Keep technical terms like 'paper', 'product', 'fellowship' in English where natural, but the sentence structure should be Chinese."
        if lang == "zh"
...
def _call_assistant_llm(message: str, lang: str, profile: Dict[str, Any], history: Dict[str, Any], ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    fellowship_reply = _fellowship_program_reply(message, lang)
    if fellowship_reply and not _has_explicit_command_intent(message):
        return fellowship_reply
