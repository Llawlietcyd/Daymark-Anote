...
def test_demo_workspace_login_seeds_fellowship_account():
    client.headers.pop("X-Session-Token", None)
    _rate_buckets.clear()

    demo = client.post("/api/session/demo")
    assert demo.status_code == 200
    body = demo.json()
    assert body["logged_in"] is True
    assert body["display_name"] == "fellow-demo"
    assert body["onboarding_completed"] is True

    client.headers["X-Session-Token"] = body["session_token"]
    tasks = client.get("/api/tasks?status=all&q=Fellowship:")
    assert tasks.status_code == 200
    titles = {task["title"] for task in tasks.json()}
    assert "Fellowship: Submit research topic proposal" in titles
    assert "Fellowship: Prepare final presentation" in titles

    review = client.get(f"/api/review-insights?date={local_date_offset_iso(0)}&month={local_date_offset_iso(0)[:7]}&lang=en")
    assert review.status_code == 200


def test_demo_workspace_does_not_seed_fake_activity_on_today():
    client.headers.pop("X-Session-Token", None)
    _rate_buckets.clear()
    ...
    today_key = local_date_offset_iso(0)

    history = client.get("/api/history?limit=200&offset=0")
    assert history.status_code == 200
    today_history = [entry for entry in history.json() if entry["date"] == today_key]
    assert today_history == []

    focus = client.get("/api/focus/history?days=7")
    assert focus.status_code == 200
    today_focus = [entry for entry in focus.json() if entry["date"] == today_key]
    assert today_focus == []
    ...
    today_due = [
        ...
        if task.get("title", "").startswith("Take-home:") and task.get("due_date") == today_key
    ]
    assert today_due == []
...
def test_list_tasks_keeps_explicit_weekly_fellowship_task_weekly():
    login_as(unique_username("weekly-fellowship-task"), "weekly-pass")
    ...
    listed = client.get("/api/tasks?status=all&q=Fellowship:")
    assert listed.status_code == 200
    task = next(item for item in listed.json() if item["title"] == "Fellowship: Join weekly AI Research Standup")
    assert task["task_kind"] == "weekly"
    assert task["recurrence_weekday"] == 1
...
def test_assistant_answers_fellowship_midpoint_question_without_creating_task():
    login_as(unique_username("assistant-fellowship-midpoint"), "assistant-pass")
    ensure_assistant_ready("en")

    created = client.post("/api/tasks", json={"title": "Draft abstract", "task_kind": "temporary"})
    assert created.status_code == 201

    before = client.get("/api/tasks?status=all")
    assert before.status_code == 200

    reply = client.post("/api/assistant/chat", json={"message": "When is the midpoint check-in?", "lang": "en"})
    assert reply.status_code == 200
    body = reply.json()

    assistant_messages = [message["content"] for message in body["messages"] if message["role"] == "assistant"]
    assert any("2026-07-01" in message and "Natan" in message for message in assistant_messages)
    assert not any("Added task:" in message or "Updated task:" in message for message in assistant_messages)

    after = client.get("/api/tasks?status=all")
    assert after.status_code == 200
    assert len(after.json()) == len(before.json())


def test_assistant_answers_fellowship_deliverables_question_without_mutating_tasks():
    login_as(unique_username("assistant-fellowship-deliverables"), "assistant-pass")
    ensure_assistant_ready("en")

    reply = client.post(
        "/api/assistant/chat",
        json={"message": "What are the 3 fellowship deliverables?", "lang": "en"},
    )
    assert reply.status_code == 200
    body = reply.json()

    assistant_messages = [message["content"] for message in body["messages"] if message["role"] == "assistant"]
    assert any("research paper" in message.lower() and "product contribution" in message.lower() for message in assistant_messages)
    assert not any("Added task:" in message or "Updated task:" in message for message in assistant_messages)
...
def test_assistant_today_in_fellowship_uses_correct_weekday_mapping(monkeypatch):
    login_as(unique_username("assistant-fellowship-today"), "assistant-pass")
    ensure_assistant_ready("en")

    monkeypatch.setattr("api_v2.routers.assistant.local_today_iso", lambda: "2026-06-02")

    reply = client.post("/api/assistant/chat", json={"message": "today in the fellowship", "lang": "en"})
    assert reply.status_code == 200
    assistant_messages = [message["content"] for message in reply.json()["messages"] if message["role"] == "assistant"]
    assert any("AI Research Standup" in message for message in assistant_messages)
    assert not any("AI Product Standup" in message or "Fellowship Talk" in message for message in assistant_messages)


def test_assistant_task_question_about_final_presentation_is_not_hijacked_by_fellowship_fast_path():
    login_as(unique_username("assistant-final-presentation-task"), "assistant-pass")
    ensure_assistant_ready("en")

    created = client.post(
        "/api/tasks",
        json={"title": "final presentation", "task_kind": "temporary", "due_date": "2026-09-01"},
    )
    assert created.status_code == 201

    reply = client.post("/api/assistant/chat", json={"message": "When is my final presentation?", "lang": "en"})
    assert reply.status_code == 200
    assistant_messages = [message["content"] for message in reply.json()["messages"] if message["role"] == "assistant"]
    assert any("2026-09-01" in message for message in assistant_messages)
    assert not any("3 core deliverables" in message or "research paper" in message for message in assistant_messages)
